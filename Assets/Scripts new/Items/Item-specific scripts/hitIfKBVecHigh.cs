...
public class hitIfKBVecHigh : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D col)
    {
        if (col.gameObject.tag == "Hostile" && col.gameObject != responsible)
        {
            ...
            gameObject.GetComponent<HPDamageDie>().Hurty(damageAmt, false, true, 1, (int)DAMAGETYPES.NORMAL, false);
        ...
        if (col.gameObject.tag == "Wall")
        {
            ...
            gameObject.GetComponent<HPDamageDie>().Hurty(0.5f * damageAmt, false, true, 1, (int)DAMAGETYPES.NORMAL, false);
            gameObject.GetComponent<NewPlayerMovement>().knockBackVector = new Vector2(0, 0);
        }
    }
}
