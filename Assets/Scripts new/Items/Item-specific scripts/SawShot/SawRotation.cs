...
public class SawRotation : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (timer % 10 == 0)
        {
            ...
            guyLatchedTo.GetComponent<HPDamageDie>().Hurty(damageAmount, isCrit, 1, (int)DAMAGETYPES.NORMAL, true, gameObject, false);
            GameObject owner = gameObject.GetComponent<DealDamage>().owner;
            //gameObject.GetComponent<DealDamage>().SendRollOnHits(guyLatchedTo);
        }
