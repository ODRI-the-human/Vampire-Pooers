...
public class ItemBERSERK : ItemScript
{
    ...
    public override void OnLevel()
    {
        if (gameObject.GetComponent<LevelUp>().level % 2 == 0)
        {
            if (!isActive)
            {
                ...
                gameObject.GetComponent<Attack>().charges[0] = EntityReferencerGuy.Instance.berserkAttack.maxCharges;
                gameObject.GetComponent<Attack>().coolDowns[0] = 0;
